...
<?php if ($this->uri->segment(2) == 'register' ): ?>
  <?php if ($this->session->flashdata('pendaftaran')): 
    $message = $this->session->flashdata('pendaftaran');
    $registeredUsername = $this->session->flashdata('registered_username');
  ?>
  <script type="text/javascript">
     swal({
        title: "Username :\n<?php echo $registeredUsername; ?>",
        text: "Copy Username diatas untuk melakukan Login",
        icon: "success",
        buttons: false,
      })
      .then((value) => {
