...
class Home extends CI_Controller {
	...
	function login()
	{
		if ($this->input->post('login')) {
			...
			if (empty($username)) {
				$username_kosong = 'Username Tidak Boleh Kosong';
				$main['username_kosong'] = $username_kosong;
			...
			if (empty($password)) {
				...
				$main['password_kosong'] = $password_kosong;
				$main['main'] = 'home/menu/login';
				$main['header'] = 'Halaman Login';
				$this->load->view('home/index', $main);
				return;
	...
	function register()
	{
		if ($this->input->post('daftar')) {
			...
			$username = $namaTanpaSpasi .'_'.$id. substr($no_telpon, -2);
			$this->mhome->update('tb_pembeli',array('id' => $id),array('username' => $username));
			$this->mhome->insert('tb_user',array('id_user' => $id,'username' => $username , 'password' => $password , 'level' => 2));
			$this->session->set_flashdata('pendaftaran', 'Pendaftaran Berhasil');
			$this->session->set_flashdata('registered_username', $username);
			redirect('/home/register');

		} else {
			$main['main'] = 'home/menu/register';
			$main['header'] = 'Halaman Register';
